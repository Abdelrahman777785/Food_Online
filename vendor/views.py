...
def vprofile(request):
    ...
    if request.method == 'POST':                 # update restaurant
...
# import requests
# from foodOnline_main.settings import MAPQUEST_API_KEY
# def geocode(request):
#     if request.method == 'POST':
#         location = request.POST.get('location')
#         api_key = MAPQUEST_API_KEY
#         url = f'https://www.mapquestapi.com/geocoding/v1/address?key={api_key}&location={location}'
#         print(api_key)
#         try:
#             response = requests.get(url)
#             data = response.json()
#             coordinates = data['results'][0]['locations'][0]['latLng']
#             return JsonResponse({'latitude': coordinates['lat'], 'longitude': coordinates['lng']})
